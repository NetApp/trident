...
3. Install Trident by using the ``bundle.yaml`` file that sets up the Trident operator for the new version. Run the following command:
   ...
   .. code-block:: bash
       ...
       kubectl install -f 21.07.2/trident-installer/deploy/bundle.yaml -n trident.

As part of this step, the 21.07.2 Trident operator will identify an existing Trident installation and upgrade it to the same version as the operator.
...
2. Create the ``TridentOrchestrator`` CRD using the manifest provided with the
  ...
  .. code-block:: bash
      ...
      # Download the release required [21.07.2]
      $ mkdir 21.07.2
      $ cd 21.07.2
...
5. Install the cluster-scoped operator.
   ...
   .. code-block:: bash
     ...
     $ pwd
     $ /root/21.07.2/trident-installer
  ...
  iii. Upgrade Trident to the version of the cluster-scoped operator being used.
       In the example above, Trident was upgraded to ``21.07.2``.
...
.. code-block:: console
 ...
 $ helm upgrade <name> trident-operator-21.07.2.tgz

where ``trident-operator-21.07.2.tgz`` reflects the version that you want to upgrade to.
...
.. code-block:: console
  ...
  $ helm upgrade <name> trident-operator-21.07.2-custom.tgz --set tridentDebug=true
