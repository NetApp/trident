...
// genuinely unknown: the lookup failed and no earlier lookup cached a result. Callers must not report
// topology in that case. Return paths keep maps.Clone results as-is: nil and {} are equivalent for
// read-only CSI Segments, and a plugin with no controllerHelper has no topology source to consult.
func (p *Plugin) getTopologyLabels(ctx context.Context) (map[string]string, error) {
	...
	if err := backoff.RetryNotify(getLabels, labelsBackoff, labelsNotify); err != nil {
		topologyLabelsLock.RLock()
		cached, cacheLoaded := maps.Clone(topologyLabels), topologyLabels != nil
		topologyLabelsLock.RUnlock()

		if cacheLoaded {
			Logc(ctx).WithError(err).Debug("Unable to get topology labels from node; using cached labels.")
			return cached, nil
		}
