...
if [ $# -ne 1 ]
then
	>&2 echo "Usage:  $0 <resource-type>"
	>&2 echo "resource-type:  Type of resource; either 'volume', 'backend', or 'storageclass'.  Required."
	>&2 echo "Provide JSON description of resource to POST on stdin."
	exit 1
fi

echo "curl -XPOST -s -S -D - ${TRIDENT_IP}:${TRIDENT_PORT}/trident/v1/${1} -d @-"
echo
curl -XPOST -s -S -D - ${TRIDENT_IP}:${TRIDENT_PORT}/trident/v1/${1} -d @-
