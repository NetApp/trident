...
then
	...
	>&2 echo "resource-name:  Specific resource to delete.  Required."
	exit 1
fi

echo "curl -XDELETE -s -S -D - ${TRIDENT_IP}:${TRIDENT_PORT}/trident/v1/${1}/${2}"
echo
curl -XDELETE -s -S -D - ${TRIDENT_IP}:${TRIDENT_PORT}/trident/v1/${1}/${2}
