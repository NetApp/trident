...
func getFCPDiskSize(ctx context.Context, devicePath string) (int64, error) {
	Logc(ctx).Debug(">>>> devices_darwin.getFCPDiskSize")
	defer Logc(ctx).Debug("<<<< devices_darwin.getFCPDiskSize")
	return 0, errors.UnsupportedError("getFCPDiskSize is not supported for darwin")
}
