...
func flushOneDevice(devicePath string) error {
	fields := log.Fields{"devicePath": devicePath}
	log.WithFields(fields).Debug(">>>> osutils_linux.flushOneDevice")
	defer log.WithFields(fields).Debug("<<<< osutils_linux.flushOneDevice")
	...
	if errno != 0 {
		err := os.NewSyscallError("ioctl", errno)
		log.Error("BLKFLSBUF ioctl failed")
