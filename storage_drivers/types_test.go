...
func TestGetDriverConfigByName(t *testing.T) {
	tests := []struct {
		...
		{
			driverName:           AstraDSStorageDriverName,
			expectedDriverConfig: &AstraDSStorageDriverConfig{},
			errorExpected:        false,
		},
		{
...
// newTestAstraDSStorageDriverConfig creates a AstraDSStorageDriverConfig for testing.
func newTestAstraDSStorageDriverConfig() *AstraDSStorageDriverConfig {
	config := &AstraDSStorageDriverConfig{
		CommonStorageDriverConfig: &CommonStorageDriverConfig{
			...
			StoragePrefix:   func(s string) *string { return &s }("trident_"),
		},
		NfsMountOptions:  "",
		AutoExportPolicy: false,
		AutoExportCIDRs:  []string{},
		AstraDSStorageDriverPool: AstraDSStorageDriverPool{
			Labels:                             nil,
			Region:                             "",
			Zone:                               "",
			SupportedTopologies:                nil,
			AstraDSStorageDriverConfigDefaults: AstraDSStorageDriverConfigDefaults{},
		},
		Storage:        []AstraDSStorageDriverPool{},
		ClusterUUID:    "uuid4-1234567890987654321234567890",
		KubeSystemUUID: "uuid4-09865432123456789087654321",
...
func TestAstraDSStorageDriverConfig_String(t *testing.T) {
	config := newTestAstraDSStorageDriverConfig()
	kubeConfig := config.Kubeconfig
	configString := config.String()
	assert.NotContains(t, configString, kubeConfig)
}

func TestAstraDSStorageDriverConfig_GoString(t *testing.T) {
	config := newTestAstraDSStorageDriverConfig()
	kubeConfig := config.Kubeconfig
	configGoString := config.GoString()
	assert.NotContains(t, configGoString, kubeConfig)
}

func TestAstraDSStorageDriverConfig_InjectSecrets(t *testing.T) {
	config := newTestAstraDSStorageDriverConfig()
	...
	config = newTestAstraDSStorageDriverConfig()
	err = config.InjectSecrets(map[string]string{
		"kubeconfig-01": "kubeconfig-01",
	})
	assert.Error(t, err)
}

func TestAstraDSStorageDriverConfig_ExtractSecrets(t *testing.T) {
	kubeconfig := "kubeconfig-01"
	config := AstraDSStorageDriverConfig{
...
func TestAstraDSStorageDriverConfig_GetAndHideSensitive(t *testing.T) {
	kubeconfig := "kubeconfig-01"
	config := AstraDSStorageDriverConfig{
		Kubeconfig: kubeconfig,
	}
...
func TestAstraDSStorageDriverConfig_CheckForCRDControllerForbiddenAttributes(t *testing.T) {
	kubeconfig := "kubeconfig-01"
	config := AstraDSStorageDriverConfig{
		Kubeconfig: kubeconfig,
	}
	...
	forbiddenKeys := config.CheckForCRDControllerForbiddenAttributes()
	assert.Contains(t, forbiddenKeys, "Kubeconfig")
}

func TestAstraDSStorageDriverConfig_SpecOnlyValidation(t *testing.T) {
	kubeconfig := "kubeconfig-01"
	config := AstraDSStorageDriverConfig{
		CommonStorageDriverConfig: &CommonStorageDriverConfig{
			Credentials: map[string]string{
