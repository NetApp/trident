...
func TestOntapSanEconomyVolumeCreate(t *testing.T) {
	...
	volConfig := &storage.VolumeConfig{
		...
		InternalName: "pvc_12345678_1234_1234_1234_123456789abc",
	}
	...
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(2).Return(&api.Lun{Size: "1073741824"}, nil)
	...
	assert.True(t, strings.HasSuffix(volConfig.InternalID, "/lun/pvc_12345678_1234_1234_1234_123456789abc"))
}
...
func TestOntapSanEconomyVolumeCreate_LUNExists(t *testing.T) {
	...
	volConfig := &storage.VolumeConfig{
		...
		Encryption:   "false",
		FileSystem:   "xfs",
		InternalName: "lun_storagePrefix_vol1",
	}
	volAttrs := map[string]sa.Request{}
	luns := []api.Lun{
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			if test.lunExists {
				...
				mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(nil, errors.New(test.message))
			}

			result := d.Create(ctx, volConfig, pool1, volAttrs)

			if test.lunExists {
				assert.Equal(t, "/svm/SVM1/flexvol/volumeName/lun/lun_storagePrefix_vol1", volConfig.InternalID)
			} else {
				assert.Equal(t, "", volConfig.InternalID)
			}
...
func TestOntapSanEconomyVolumeCreate_OverPoolSizeLimit_CreateNewFlexvol(t *testing.T) {
	...
	// New concurrency implementation flow:
	// 1. First, LUNExists() is called to check if volume already exists - calls LunList (returns empty)
	// 2. findFlexvolForLUN is called twice (initial + double-check after acquiring flexvolCreationMux)
	//    Each call: VolumeListByAttrs, then for each volume: VolumeInfo + LunList (for size check)
	// 3. After creating new FlexVol, resizeFlexvol also calls VolumeInfo + LunList
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return([]api.Lun{}, nil) // LUNExists check
...
func TestOntapSanEconomyVolumeCreate_NotOverPoolSizeLimit_UseExistingFlexvol(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).AnyTimes().Return([]api.Lun{}, nil)
	mockAPI.EXPECT().VolumeListByAttrs(ctx, gomock.Any()).Times(1).Return([]*api.Volume{existingFlexvol}, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).AnyTimes().Return(existingFlexvol, nil)
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().LunCreate(ctx, gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(2).Return(&api.Lun{Size: "1073741824"}, nil)
	mockAPI.EXPECT().LunSetAttribute(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(),
...
func TestOntapSanEconomyVolumeCreate_NoFlexvol(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{}, nil)
	mockAPI.EXPECT().VolumeListByAttrs(ctx, gomock.Any()).Return(api.Volumes{}, errors.New("failed to list volumes"))
...
func TestOntapSanEconomyVolumeCreate_ResizeFailed(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			// Flow with double-check locking pattern:
			// 1. LUNExists check (LunList #1)
			// 2. findFlexvolForLUN #1 - VolumeListByAttrs #1 returns empty
			// 3. findFlexvolForLUN #2 (double-check) - VolumeListByAttrs #2 returns empty
			// 4. createFlexvolForLUN - VolumeCreate
			// 5. resizeFlexvol - VolumeInfo, LunList #2, VolumeSetSize (fails)
			...
			if test.destroyError {
				mockAPI.EXPECT().VolumeDestroy(ctx, gomock.Any(), true, true).Return(errors.New(test.message))
			} else {
...
func TestOntapSanEconomyVolumeCreate_LUNCreateFailed(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			...
			// 3. findFlexvolForLUN #2 (double-check) - VolumeListByAttrs #2 returns empty
			// 4. createFlexvolForLUN - VolumeCreate
			// 5. resizeFlexvol - VolumeInfo, LunList #2, VolumeSetSize
			// 6. LunCreate (fails)
...
func TestOntapSanEconomyVolumeCreate_TooManyLUNs(t *testing.T) {
	...
	// Flow with TooManyLunsError retry and double-check locking:
	// 1. LUNExists check (LunList #1)
	// 2. findFlexvolForLUN #1 + #2 (double-check) - VolumeListByAttrs #1, #2 returns empty
	// 3. createFlexvolForLUN - VolumeCreate #1
	// 4. resizeFlexvol - VolumeInfo #1, LunList #2, VolumeSetSize #1
	// 5. LunCreate #1 fails with TooManyLunsError
	// 6. Retry: findFlexvolForLUN #3 + #4 (double-check) - VolumeListByAttrs #3, #4 returns empty
	// 7. createFlexvolForLUN - VolumeCreate #2
	// 8. resizeFlexvol - VolumeInfo #2, LunList #3, VolumeSetSize #2
	// 9. LunCreate #2 succeeds
...
func TestOntapSanEconomyVolumeCreate_GetLUNFailed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().LunCreate(ctx, gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(1).Return(nil, errors.New("failed to fetch lun"))
...
func TestOntapSanEconomyVolumeCreate_LUNSetAttributeFailed(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			...
			mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(2).Return(&api.Lun{Size: "1073741824"}, nil)
			mockAPI.EXPECT().LunSetAttribute(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(),
				gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(errors.New("failed to set attribute"))

			switch test.errorType {
			case "Lun":
				mockAPI.EXPECT().LunDestroy(ctx, gomock.Any()).Return(errors.New(test.message))
				mockAPI.EXPECT().VolumeDestroy(ctx, gomock.Any(), true, true).Return(nil)
			case "Volume":
				mockAPI.EXPECT().LunDestroy(ctx, gomock.Any()).Return(nil)
				mockAPI.EXPECT().VolumeDestroy(ctx, gomock.Any(), true, true).Return(errors.New(test.message))
...
func TestOntapSanEconomyVolumeCreate_Resize(t *testing.T) {
	...
	mockAPI.EXPECT().LunCreate(ctx, gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(2).Return(&lun, nil)
	mockAPI.EXPECT().LunSetAttribute(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(),
		gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(nil)
...
func TestOntapSanEconomyVolumeCreate_ResizeVolumeSizeFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunSetAttribute(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(),
		gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().VolumeSize(ctx, gomock.Any()).Return(uint64(1073741824), errors.New("failed to set size"))
...
func TestOntapSanEconomyVolumeCreate_ResizeSetSizeFailed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Return(errors.New("failed to set volume size"))
...
func TestOntapSanEconomyVolumeCreate_ResizeVolumeSizeFailed2(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeSize(ctx, gomock.Any()).Return(uint64(1073741824), nil)
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Return(nil)
	mockAPI.EXPECT().VolumeSize(ctx, gomock.Any()).Return(uint64(1073741824), errors.New("failed to get volume size"))
...
func TestOntapSanEconomyVolumeCreate_FormatOptions(t *testing.T) {
	...
	mockAPI.EXPECT().LunSetAttribute(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(),
		gomock.Any(), tempFormatOptions, gomock.Any()).Times(1).Return(nil)
...
func TestOntapSanEconomyVolumeClone(t *testing.T) {
	...
	volConfig := &storage.VolumeConfig{
		...
		Encryption:                "false",
		FileSystem:                "xfs",
		Name:                      "storagePrefix-cloneName",
		InternalName:              "storagePrefix_cloneName",
	}
	sourceLun := api.Lun{
		Size:       "1073741824",
	...
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(&api.Lun{
		Name: "/vol/volumeName/storagePrefix_cloneName", Size: "1073741824", VolumeName: "volumeName",
	}, nil).Times(1)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Times(1).Return(&api.Volume{}, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{}, nil)
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Times(1).Return(nil)
	...
	assert.Equal(t, "/svm/SVM1/flexvol/volumeName/lun/storagePrefix_cloneName", volConfig.InternalID)
	assert.Nil(t, result)
}
...
func TestOntapSanEconomyVolumeClone_OverPoolSizeLimit_CheckPassed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return([]api.Lun{sourceLun}, nil)
	mockAPI.EXPECT().LunCloneCreate(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(1).Return(nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(&cloneLun, nil).Times(1)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Times(1).Return(&api.Volume{Size: "1073741824"}, nil)
...
func TestOntapSanEconomyVolumeImport(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				mockAPI.EXPECT().VolumeInfo(gomock.Any(), gomock.Any()).Return(nil, errors.New("import is not implemented"))
			},
			wantErr: assert.Error,
			testOut: "Import succeeded",
		},
		...
		},
		{
			...
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				...
				mockAPI.EXPECT().LunGetByName(gomock.Any(), gomock.Any()).Return(nil, errors.New("LUN not found"))
			},
			wantErr: assert.Error,
			testOut: "Import succeeded",
		},
...
func TestOntapSanEconomyVolumeImport_Managed(t *testing.T) {
	...
	volConfig := &storage.VolumeConfig{
		InternalName: "pvc-1234",
		Size:         "1g",
		Encryption:   "false",
		FileSystem:   "xfs",
	}
	d.flexvolNamePrefix = "test_lun_pool_"
	originalFlexvol := &api.Volume{
		AccessType: "rw",
		Name:       "my_vol",
	}
	...
	tests := []struct {
		...
		{
			name:        "VolumeInfo_managedImport_LUNRenameFail",
			volConfig:   volConfig.ConstructClone(),
			volToImport: "my_vol/my_LUN",
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				mockAPI.EXPECT().VolumeInfo(gomock.Any(), gomock.Any()).Return(originalFlexvol, nil)
				mockAPI.EXPECT().LunGetByName(gomock.Any(), gomock.Any()).
				...
				mockAPI.EXPECT().LunRename(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(errors.New("LUN rename failed"))
			},
			wantErr: assert.Error,
			testOut: "Import succeeded",
		},
		{
			name:        "VolumeInfo_managedImport_VolumeRenameFailed",
			volConfig:   volConfig.ConstructClone(),
			volToImport: "my_vol/my_LUN",
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				mockAPI.EXPECT().VolumeInfo(gomock.Any(), gomock.Any()).Return(originalFlexvol, nil)
				mockAPI.EXPECT().LunGetByName(gomock.Any(), gomock.Any()).
					Return(&api.Lun{Name: "/vol/my_vol/importMe", State: "online", Size: "1073741824"}, nil)
				...
				mockAPI.EXPECT().VolumeRename(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(errors.New("volume rename failed"))
			},
			wantErr: assert.Error,
			testOut: "Import succeeded",
		},
		{
			name:        "VolumeInfo_managedImport_LUNNameRestoreFailed",
			volConfig:   volConfig.ConstructClone(),
			volToImport: "my_vol/my_LUN",
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				mockAPI.EXPECT().VolumeInfo(gomock.Any(), gomock.Any()).Return(originalFlexvol, nil)
		...
		{
			name:        "VolumeInfo_managedImport_igroupListFailed",
			volConfig:   volConfig.ConstructClone(),
			volToImport: "my_vol/my_LUN",
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				mockAPI.EXPECT().VolumeInfo(gomock.Any(), gomock.Any()).Return(originalFlexvol, nil)
				mockAPI.EXPECT().LunGetByName(gomock.Any(), gomock.Any()).
					Return(&api.Lun{Name: "/vol/my_LUN/my_LUN", State: "online", Size: "1073741824"}, nil)
				mockAPI.EXPECT().LunRename(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
		...
		{
			name: "VolumeInfo_managedImport_renameNotNecessary",
			volConfig: &storage.VolumeConfig{
				InternalName: "my_LUN",
				Size:         "1g",
				Encryption:   "false",
			...
			testOut: "Import succeeded",
			validate: func(t *testing.T, volConfig *storage.VolumeConfig) {
				assert.Equal(t, "/svm/SVM1/flexvol/my_vol/lun/my_LUN", volConfig.InternalID)
			},
		},
		{
			name:        "VolumeInfo_managedImport_lunAndFlexvolRename",
			volConfig:   volConfig.ConstructClone(),
			volToImport: "my_vol/my_LUN",
			mocks: func(mockAPI *mockapi.MockOntapAPI) {
				mockAPI.EXPECT().VolumeInfo(gomock.Any(), gomock.Any()).Return(originalFlexvol, nil)
			...
			testOut: "Import succeeded",
			validate: func(t *testing.T, volConfig *storage.VolumeConfig) {
				assert.True(t, strings.HasPrefix(volConfig.InternalID, "/svm/SVM1/flexvol/test_lun_pool_"),
					"InternalID should have correct prefix")
				assert.True(t, strings.HasSuffix(volConfig.InternalID, "/lun/pvc-1234"),
					"InternalID should have correct suffix")
			},
		},
	}
	for _, test := range tests {
...
func economyMainLUNPath(bucketVol string) string {
	return fmt.Sprintf("/vol/%s/storagePrefix_vol1", bucketVol)
}

// expectDestroyEconomySnapDelete mocks DeleteSnapshot when invoked from Destroy.
// Callers must already expect LunList for snapshot LUN existence before this helper.
func expectDestroyEconomySnapDelete(
	mockAPI *mockapi.MockOntapAPI,
	bucketVol string,
	snapLun *api.Lun,
	flexVol *api.Volume,
	remainingLuns api.Luns,
) {
	mockAPI.EXPECT().LunGetByName(ctx, snapLun.Name).Return(snapLun, nil)
	volumeInfoCalls := 1
	if len(remainingLuns) > 0 && !isAutosizeOnlyGrow(flexVol) {
		// Pre-delete VolumeInfo plus resizeFlexvol optimal sizing when not grow-only mode.
		volumeInfoCalls = 2
	}
	mockAPI.EXPECT().VolumeInfo(ctx, bucketVol).Return(flexVol, nil).Times(volumeInfoCalls)
	mockAPI.EXPECT().LunDestroy(ctx, gomock.Any()).Return(nil)
	mockAPI.EXPECT().LunList(ctx, economyFlexvolLUNListPath(bucketVol)).Return(remainingLuns, nil).Times(1)
	if len(remainingLuns) > 0 {
		mockAPI.EXPECT().VolumeSetSize(ctx, bucketVol, gomock.Any()).Return(nil).Times(1)
	} else {
		mockAPI.EXPECT().VolumeDestroy(ctx, bucketVol, true, true).Return(nil)
	}
}

// expectDestroyEconomyMainLUNDelete mocks main LUN teardown (pre-delete VolumeInfo, unmap, destroy)
// and empty-bucket VolumeDestroy.
func expectDestroyEconomyMainLUNDelete(
	mockAPI *mockapi.MockOntapAPI,
	bucketVol string,
	mainLun *api.Lun,
	flexVol *api.Volume,
) {
	mockAPI.EXPECT().LunGetByName(ctx, economyMainLUNPath(bucketVol)).Return(mainLun, nil)
...
func TestOntapSanEconomyVolumeDestroy(t *testing.T) {
	...
	mainLun := api.Lun{Size: "1073741824", Name: "lun_storagePrefix_vol1", VolumeName: "volumeName"}
	snapLun := api.Lun{
		Size: "1073741824", Name: "/vol/volumeName/storagePrefix_vol1_snapshot_mySnap", VolumeName: "volumeName",
	}
	snapLuns := []api.Lun{snapLun}
	flexVol := &api.Volume{Name: "flexvol", Size: "2147483648", Comment: "flexvol"}

	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{mainLun}, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(snapLuns, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{snapLun}, nil)
	expectDestroyEconomySnapDelete(mockAPI, "volumeName", &snapLun, flexVol, api.Luns{mainLun})
	expectDestroyEconomyMainLUNDelete(mockAPI, "volumeName", &mainLun, flexVol)
...
func TestOntapSanEconomyVolumeDestroy_InternalID(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	volConfig := &storage.VolumeConfig{
		InternalName: "storagePrefix_vol1",
		InternalID:   "/svm/SVM1/flexvol/volumeName/lun/storagePrefix_vol1",
		Size:         "1g",
		Encryption:   "false",
		FileSystem:   "xfs",
	}
	...
	mockAPI.EXPECT().LunList(ctx, "/vol/volumeName/storagePrefix_vol1").Return(luns, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(snapLuns, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{snapLun}, nil)
	expectDestroyEconomySnapDelete(mockAPI, "volumeName", &snapLun, flexVol, luns)
	expectDestroyEconomyMainLUNDelete(mockAPI, "volumeName", &mainLun, flexVol)
...
func TestOntapSanEconomyVolumeDestroy_DeleteSnapshotFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunDestroy(ctx, gomock.Any()).Return(errors.New("failed to delete lun"))
...
func TestOntapSanEconomyVolumeDestroy_UnmapFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{snapLun}, nil)
	expectDestroyEconomySnapDelete(mockAPI, "volumeName", &snapLun, flexVol, luns)
	mockAPI.EXPECT().LunGetByName(ctx, economyMainLUNPath("volumeName")).Return(&mainLun, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, "volumeName").Return(flexVol, nil)
	mockAPI.EXPECT().LunListIgroupsMapped(ctx, gomock.Any()).Return([]string{"igroup1"}, nil)
	mockAPI.EXPECT().LunUnmap(ctx, "igroup1", gomock.Any()).Return(errors.New("failed to unmap lun"))
...
func TestOntapSanEconomyVolumeDestroy_LUNDestroyFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunListIgroupsMapped(ctx, gomock.Any()).Return([]string{"igroup1"}, nil)
	mockAPI.EXPECT().LunUnmap(ctx, "igroup1", gomock.Any()).Return(nil)
	mockAPI.EXPECT().LunDestroy(ctx, gomock.Any()).Return(errors.New("failed to destroy lun"))
...
func TestOntapSanEconomyVolumeDestroy_DockerContext_Failure(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(luns, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(snapLuns, nil).AnyTimes()
	mockAPI.EXPECT().IscsiNodeGetNameRequest(ctx).Return("", errors.New("error fetching node name"))
...
func TestOntapSanEconomyVolumeDestroy_DockerContext_LunMapInfoError(t *testing.T) {
	...
	mockAPI.EXPECT().IscsiNodeGetNameRequest(ctx).Return("node", nil)
	mockAPI.EXPECT().IscsiInterfaceGet(ctx, "SVM1").Return([]string{"node"}, nil)
	mockAPI.EXPECT().LunMapInfo(ctx, "", gomock.Any()).Return(0, errors.New("failed to fetch lun map info"))
...
func TestOntapSanEconomyVolumeDeleteBucketIfEmpty_Fsx(t *testing.T) {
	...
	result := d.deleteBucketIfEmpty(ctx, bucketVol, 0, nil)
...
func TestOntapSanEconomyVolumeDeleteBucketIfEmpty(t *testing.T) {
	...
	result := d.deleteBucketIfEmpty(ctx, "volumeName", 0, nil)
...
func TestOntapSanEconomyVolumeDeleteBucketIfEmpty_LUNFetchFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(nil, errors.New("error fetching lun"))
...
func TestOntapSanEconomyVolumeDeleteBucketIfEmpty_LUNDestroyFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{}, nil)
	mockAPI.EXPECT().VolumeDestroy(ctx, "volumeName", true, true).Return(errors.New("failed to destroy lun"))
...
func TestOntapSanEconomyVolumeDeleteBucketIfEmpty_GrowModeShrinksByDeletedLUNSize(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	const (
		flexvolName              = "volumeName"
	...
	luns := []api.Lun{
		{Size: "1073741824", Name: "lun_storagePrefix_vol2", VolumeName: flexvolName},
	}

	preDeleteVol := &api.Volume{
		Name:         flexvolName,
		Size:         strconv.FormatUint(flexvolBytesBeforeDelete, 10),
		AutosizeMode: api.VolumeAutosizeModeGrow,
	}

	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(luns, nil)
	mockAPI.EXPECT().VolumeSetSize(
		ctx, flexvolName, strconv.FormatUint(flexvolBytesAfterDelete, 10),
	).Return(nil)

	result := d.deleteBucketIfEmpty(ctx, flexvolName, deletedLunBytes, preDeleteVol)
...
func TestOntapSanEconomyVolumeDeleteBucketIfEmpty_VolumeSetSizeFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(luns, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, "volumeName").Return(nil, errors.New("failed to fetch volume info"))
...
func TestOntapSanEconomyVolumePublish(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, "/vol/*/lunName").Times(1).Return(api.Luns{api.Lun{Size: "1g", Name: "lunName", VolumeName: "volumeName"}}, nil)
	...
	assert.NoError(t, result)
	assert.Equal(t, "/svm/SVM1/flexvol/volumeName/lun/lunName", volConfig.InternalID)
}

func TestOntapSanEconomyVolumePublish_InternalID(t *testing.T) {
	originalContext := tridentconfig.CurrentDriverContext
	tridentconfig.CurrentDriverContext = tridentconfig.ContextCSI
	defer func() { tridentconfig.CurrentDriverContext = originalContext }()
	...
	volConfig := &storage.VolumeConfig{
		InternalName:     "lunName",
		InternalID:       "/svm/SVM1/flexvol/volumeName/lun/lunName",
		Size:             "1g",
		Encryption:       "false",
		FileSystem:       "xfs",
		FormatOptions:    "-b 4096",
	...
	mockAPI.EXPECT().LunList(ctx, "/vol/volumeName/lunName").Times(1).Return(api.Luns{api.Lun{Size: "1g", Name: "lunName", VolumeName: "volumeName"}}, nil)
	mockAPI.EXPECT().IgroupCreate(ctx, gomock.Any(), "iscsi", "linux").Return(nil)
	mockAPI.EXPECT().IscsiNodeGetNameRequest(ctx).Times(1).Return("node1", nil)
	mockAPI.EXPECT().IscsiInterfaceGet(ctx, gomock.Any()).Return([]string{"iscsi_if"}, nil).Times(1)
	mockAPI.EXPECT().LunGetFSType(ctx, "/vol/volumeName/lunName").Return("xfs", nil)
...
func TestOntapSanEconomyVolumePublish_GetISCSITargetInfoFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx,
		gomock.Any()).Times(1).Return(api.Luns{api.Lun{Size: "1g", Name: "lunName", VolumeName: "volumeName"}}, nil)
	mockAPI.EXPECT().IscsiNodeGetNameRequest(ctx).Times(1).Return("", errors.New("failed to get name"))
...
func TestOntapSanEconomyVolumeUnpublishX(t *testing.T) {
	...
	volConfig := &storage.VolumeConfig{
		InternalName: "lun0",
		InternalID:   "/svm/SVM1/flexvol/bucket0/lun/lun0",
		AccessInfo:   models.VolumeAccessInfo{PublishEnforcement: true},
	}
	publishInfo := &models.VolumePublishInfo{
		HostName:         "bar",
	...
	mockAPI.EXPECT().LunList(ctx, "/vol/bucket0/lun0").Return(luns, nil)
	mockAPI.EXPECT().LunMapInfo(ctx, igroupName, lunPath).Return(0, nil)
	mockAPI.EXPECT().LunUnmap(ctx, igroupName, lunPath).Return(nil)
	mockAPI.EXPECT().IgroupListLUNsMapped(ctx, igroupName).Return(nil, nil)
...
func TestOntapSanEconomyCreateSnapshot_LUNCreateCloneFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunCloneCreate(ctx, gomock.Any(), gomock.Any(), gomock.Any(),
		api.QosPolicyGroup{}).Return(errors.New("failed to create lun clone"))
...
func TestOntapSanEconomyCreateSnapshot_LUNGetByNameFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return([]api.Lun{sourceLun}, nil).Times(1)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(nil, errors.New("failed to fetch lun"))
...
func TestOntapSanEconomyCreateSnapshot_CreateLUNClone_LUNDoesNotExist(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			...
			internalID, result := d.createLUNClone(ctx, "storagePrefix_pvc_ff297a18_921a_4435_b679_c3fea351f92c",
				"source", "snap", &d.Config, mockAPI, "storagePrefix", true, policy)

			assert.Equal(t, "", internalID)
			assert.Error(t, result)
		})
	}
}
...
func TestOntapSanEconomyCreateSnapshot_CreateLUNClone_LUNCreatedFromSnapshot(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			...
			internalID, result := d.createLUNClone(ctx, "lun_storagePrefix_vol1", "source", "snap", &d.Config, mockAPI,
...
func TestOntapSanEconomyCreateSnapshot_CreateLUNClone_LUNCloneCreateFailed(t *testing.T) {
	...
	internalID, result := d.createLUNClone(ctx,
...
func TestOntapSanEconomyVolumeRestoreSnapshot(t *testing.T) {
	...
	// Preliminary check for volLun
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{lun}, nil)
	// Re-validation check for volLun (after lock)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{lun}, nil)
	// Check for snapLun
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{snapLun}, nil)
	// Check for snapLunCopy (doesn't exist)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{}, nil)
	...
	result := d.RestoreSnapshot(ctx, snapConfig, &storage.VolumeConfig{})

	assert.NoError(t, result)
}

func TestOntapSanEconomyVolumeRestoreSnapshot_InternalID(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	...
	// Preliminary check for volLun (with InternalID path)
	mockAPI.EXPECT().LunList(ctx, GetLUNPathEconomy(bucketVol, snapConfig.VolumeInternalName)).Times(1).Return(api.Luns{lun}, nil)
	// Re-validation check for volLun (after lock, with InternalID path)
	mockAPI.EXPECT().LunList(ctx, GetLUNPathEconomy(bucketVol, snapConfig.VolumeInternalName)).Times(1).Return(api.Luns{lun}, nil)
	// Check for snapLun
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{snapLun}, nil)
	// Check for snapLunCopy (doesn't exist)
	...
	result := d.RestoreSnapshot(ctx, snapConfig, sourceVolConfig)
...
func TestOntapSanEconomyVolumeRestoreSnapshot_NonexistentSnapLun(t *testing.T) {
	...
	// Re-validation check for volLun (after lock)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{lun}, nil)
	// Check for snapLun (doesn't exist)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{}, nil)
...
func TestOntapSanEconomyVolumeRestoreSnapshot_DifferentFlexvols(t *testing.T) {
	...
	// Re-validation check for volLun (after lock)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{lun}, nil)
	// Check for snapLun (in different FlexVol)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{snapLun}, nil)
...
func TestOntapSanEconomyVolumeRestoreSnapshot_SnapLunCopyExistsDifferentFlexvols(t *testing.T) {
	...
	// Check for snapLun
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{snapLun}, nil)
	// Check for snapLunCopy (exists in different FlexVol)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Times(1).Return(api.Luns{snapLunCopy}, nil)
...
func TestOntapSanEconomyVolumeDeleteSnapshot(t *testing.T) {
	...
	snapConfig := &storage.SnapshotConfig{
		...
		InternalName:       "snap_1",
		VolumeName:         "my_Lun",
		VolumeInternalName: "storagePrefix_my_Lun",
	}
	sourceLun := api.Lun{
		Size:       "1073741824",
		Name:       "storagePrefix_my_Lun",
	...
	snapPath := "/vol/my_Bucket/storagePrefix_my_Lun_snapshot_snap_1"

	mockAPI.EXPECT().LunList(ctx, "/vol/*/storagePrefix_my_Lun_snapshot_snap_1").Return(api.Luns{snapshotLun}, nil)
	mockAPI.EXPECT().LunGetByName(ctx, snapPath).Return(&snapshotLun, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, "my_Bucket").Return(&api.Volume{Size: "1073741824"}, nil).Times(2)
	mockAPI.EXPECT().LunDestroy(ctx, snapPath)
	mockAPI.EXPECT().LunList(ctx, "/vol/my_Bucket/*").Return(api.Luns{sourceLun}, nil)
...
func TestOntapSanEconomyVolumeDeleteSnapshot_LUNDoesNotExist(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.message, func(t *testing.T) {
			...
			result := d.DeleteSnapshot(ctx, snapConfig, &storage.VolumeConfig{})
...
func TestOntapSanEconomyEnsureFlexvolForLUN_InvalidVolumeSize(t *testing.T) {
	...
	flexVol, newly, err := d.ensureFlexvolForLUN(ctx, &api.Volume{}, uint64(1073741824), opts, &d.Config, pool1, make(map[string]struct{}))

	assert.Nil(t, flexVol)
...
func TestOntapSanEconomyEnsureFlexvolForLUN_FlexvolNotFound(t *testing.T) {
	...
	// Double-check locking pattern: VolumeListByAttrs called twice when no FlexVol found
	// 1. First call to findFlexvolForLUN - returns empty
...
func TestOntapSanEconomyEnsureFlexvolForLUN_NewFlexvolNotPermitted(t *testing.T) {
	...
	assert.Nil(t, flexVol, "Expected no Flexvol, got non-nil")
	assert.False(t, newly)
...
func TestGetVolumeForImport_RetriesLunNotFound(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)

	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Times(1).Return(&api.Volume{Aggregates: []string{"data"}}, nil)
	var calls int
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(2).DoAndReturn(
		func(_ context.Context, path string) (*api.Lun, error) {
			calls++
			if calls == 1 {
				return nil, utilserrors.NotFoundError("could not get LUN by name %v", path)
			}
			return &api.Lun{
				Size:       "1073741824",
				Name:       "/vol/my_Bucket/storagePrefix_my_Lun_my_Bucket",
...
func TestGetVolumeForImport_LUNDoesNotExist(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(nil, errors.New("failed to get volume"))
...
func TestGetVolumeForImport_VolumeInfoFailed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Times(1).Return(nil, errors.New("failed to get volume info"))
...
func TestGetVolumeForImport_LunGetByNameFailed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Times(1).Return(&api.Volume{Aggregates: []string{"data"}}, nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(nil, errors.New("failed to get lun by name"))
...
func TestGetVolumeExternalWrappers_Failed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeListByPrefix(ctx, gomock.Any()).Return(nil, errors.New("failed to list volume by prefix"))
...
func TestGetVolumeExternalWrappers_LUNDoesNotExist(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(nil, errors.New("failed to fetch lun"))
	mockAPI.EXPECT().VolumeListByPrefix(ctx, gomock.Any()).Return(api.Volumes{&api.Volume{}}, nil).Times(1)
...
func TestOntapSanEconomy_LUNExists(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)

	mockAPI.EXPECT().LunList(ctx, "/vol/storagePrefix*/storagePrefix_lun0").Return(nil, nil)
	_, _, _ = d.LUNExists(ctx, "storagePrefix_lun0", "", "storagePrefix")

	mockAPI.EXPECT().LunList(ctx, "/vol/my_Bucket/storagePrefix_lun0").Return(nil, nil)
	_, _, _ = d.LUNExists(ctx, "storagePrefix_lun0", "/svm/SVM1/flexvol/my_Bucket/lun/storagePrefix_lun0", "storagePrefix")

	mockAPI.EXPECT().LunList(ctx, "/vol/storagePrefix*/storagePrefix_lun0").Return(nil, nil)
	_, _, _ = d.LUNExists(ctx, "storagePrefix_lun0", "/invalid/internalID", "storagePrefix")
}

func TestOntapSanEconomyVolumeResize(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.flexvolNamePrefix = "storagePrefix_"
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	volConfig := &storage.VolumeConfig{
		Size:         "1073741824",
		Encryption:   "false",
		FileSystem:   "xfs",
		Name:         "my_Lun",
		InternalName: "storagePrefix_my_Lun",
	}
	lun := api.Lun{
		Size:       "1073741824",
		Name:       "/vol/storagePrefix_my_Bucket/storagePrefix_my_Lun",
		VolumeName: "storagePrefix_my_Bucket",
	}

	mockAPI.EXPECT().LunList(ctx, "/vol/storagePrefix_*/storagePrefix_my_Lun").Return(api.Luns{lun}, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{lun}, nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(1).Return(&lun, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Size: "1073741824"}, nil).Times(1)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Aggregates: []string{"data"}}, nil).Times(1)
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Times(1).Return(nil)
	...
	assert.Nil(t, result)
	assert.Equal(t, "/svm/SVM1/flexvol/storagePrefix_my_Bucket/lun/storagePrefix_my_Lun", volConfig.InternalID)
}

func TestOntapSanEconomyVolumeResize_InternalID(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.flexvolNamePrefix = "storagePrefix_"
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	volConfig := &storage.VolumeConfig{
		...
		Name:         "my_Lun",
		InternalName: "storagePrefix_my_Lun",
		InternalID:   "/svm/SVM1/flexvol/storagePrefix_my_Bucket/lun/storagePrefix_my_Lun",
	}
	lun := api.Lun{
		Size:       "1073741824",
		Name:       "/vol/storagePrefix_my_Bucket/storagePrefix_my_Lun",
	...
	mockAPI.EXPECT().LunList(ctx, "/vol/storagePrefix_my_Bucket/storagePrefix_my_Lun").Return(api.Luns{lun}, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(api.Luns{lun}, nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Times(1).Return(&lun, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Size: "1073741824"}, nil).Times(1)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Aggregates: []string{"data"}}, nil).Times(1)
...
func TestOntapSanEconomyVolumeResize_InvalidLUNSize(t *testing.T) {
	...
	volConfig := &storage.VolumeConfig{
		...
		FileSystem:   "xfs",
		Name:         "lun-vol1",
		InternalName: "lun_vol1",
		InternalID:   "/svm/SVM1/flexvol/volumeName/lun/lun_vol1",
	}
	luns := []api.Lun{
		{
	...
	mockAPI.EXPECT().LunList(ctx, "/vol/volumeName/lun_vol1").Return(luns, nil)
	mockAPI.EXPECT().LunList(ctx, "/vol/volumeName/*").Return(luns, nil)
...
func TestOntapSanEconomyVolumeResize_VolumeInfoFailed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(nil, errors.New("failed to get volume info")).Times(2)
...
func TestOntapSanEconomyVolumeResize_OptimalUnavailableUsesLUNSum(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.flexvolNamePrefix = "storagePrefix_"
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	volConfig := &storage.VolumeConfig{
	...
	const (
		bucketVol       = "storagePrefix_my_Bucket"
		currentLunBytes = uint64(1_073_741_824)
		requestedBytes  = uint64(2_147_483_648)
	)
	lun := api.Lun{
		Size:       strconv.FormatUint(currentLunBytes, 10),
		Name:       "/vol/storagePrefix_my_Bucket/storagePrefix_my_Lun",
		VolumeName: bucketVol,
	}
	expectedFlexvolSize := strconv.FormatUint(requestedBytes, 10)

	mockAPI.EXPECT().LunList(ctx, "/vol/storagePrefix_*/storagePrefix_my_Lun").Return(api.Luns{lun}, nil)
	mockAPI.EXPECT().LunList(ctx, "/vol/storagePrefix_my_Bucket/*").Return(api.Luns{lun}, nil)
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(&lun, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, bucketVol).Return(nil, errors.New("volume info failed")).Times(1)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Aggregates: []string{"data"}}, nil).Times(1)
	mockAPI.EXPECT().VolumeSetSize(ctx, bucketVol, expectedFlexvolSize).Return(nil)
	mockAPI.EXPECT().LunSetSize(ctx, gomock.Any(), expectedFlexvolSize).Return(requestedBytes, nil)

	err := d.Resize(ctx, volConfig, requestedBytes)
	assert.NoError(t, err)
}
...
func TestOntapSanEconomyVolumeResize_VolumeSetSizeFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(&lun, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Size: "1073741824"}, nil).Times(1)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(&api.Volume{Aggregates: []string{"aggr"}}, nil).Times(1)
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Return(errors.New("failed to set volume size"))
...
func TestOntapSanEconomyVolumeResize_LunSetSizeFailed(t *testing.T) {
	...
	mockAPI.EXPECT().LunSetSize(ctx, gomock.Any(), "2147483648").Return(uint64(2147483648),
		errors.New("failed to set lun size"))
...
func TestOntapSanEconomyVolumeResize_VolumeSizeFailed(t *testing.T) {
	...
	mockAPI.EXPECT().VolumeSetSize(ctx, gomock.Any(), gomock.Any()).Return(nil).Times(2)
	mockAPI.EXPECT().LunSetSize(ctx, gomock.Any(), gomock.Any()).Return(uint64(16106127360), nil)
	mockAPI.EXPECT().VolumeSize(ctx, gomock.Any()).Return(uint64(2147483648), errors.New("failed to get volume size"))
...
func TestOntapSanEconomyInitialize_FindFlexvolForLUN(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	d.lunsPerFlexvol = 1
	vol := &api.Volume{
	...
	lockedFlexvol, err := d.findFlexvolForLUN(ctx, vol, uint64(1073741824), true, uint64(2147483648), make(map[string]struct{}))
	if lockedFlexvol != nil {
		defer lockedFlexvol.Unlock()
	}

	assert.NoError(t, err)
	assert.NotNil(t, lockedFlexvol)
}

func TestOntapSanEconomyInitialize_FindFlexvolForLUN_InvalidLUNPath(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	d.lunsPerFlexvol = 1
	vol := &api.Volume{
	...
	lockedFlexvol, err := d.findFlexvolForLUN(ctx, vol, uint64(1073741824), false, uint64(2147483648), make(map[string]struct{}))
	if lockedFlexvol != nil {
		defer lockedFlexvol.Unlock()
	}

	assert.NoError(t, err)
	assert.Nil(t, lockedFlexvol)
}

func TestOntapSanEconomyInitialize_FindFlexvolForLUN_LUNDoesNotExist(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	d.lunsPerFlexvol = 1
	vol := &api.Volume{
	...
	mockAPI.EXPECT().VolumeListByAttrs(ctx, gomock.Any()).Return(api.Volumes{vol}, nil)
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(nil, errors.New("error fetching luns"))
...
func TestOntapSanEconomyInitialize_FindFlexvolForLUN_LimitFlexvolSize_Failed(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	d.lunsPerFlexvol = 1
	vol := &api.Volume{
	...
	mockAPI.EXPECT().VolumeListByAttrs(ctx, gomock.Any()).Return(api.Volumes{vol}, nil)
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Times(1).Return(nil, errors.New("failed to get volume"))
...
func TestOntapSanEconomyInitialize_FindFlexvolForLUN_LargeSize(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	d.lunsPerFlexvol = 1
	vol := &api.Volume{
	...
	lockedFlexvol, err := d.findFlexvolForLUN(ctx, vol, uint64(1073741824), true, uint64(1073741824), make(map[string]struct{}))
	if lockedFlexvol != nil {
		defer lockedFlexvol.Unlock()
	}
...
func TestOntapSanEconomyInitialize_FindFlexvolForLUN_MultipleVolumes(t *testing.T) {
	mockAPI, d := newMockOntapSanEcoDriver(t)
	d.helper = NewTestLUNHelper("storagePrefix_", tridentconfig.ContextCSI)
	d.lunsPerFlexvol = 1
	vol1 := &api.Volume{
	...
	// With shuffle, we might check vol1 or vol2 first. Use AnyTimes() for non-deterministic order
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(luns, nil).AnyTimes()
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(vol1, nil).AnyTimes()
	mockAPI.EXPECT().VolumeInfo(ctx, gomock.Any()).Return(vol2, nil).AnyTimes()

	lockedFlexvol, err := d.findFlexvolForLUN(ctx, vol1, uint64(1073741824), true, uint64(2147483648), make(map[string]struct{}))
	if lockedFlexvol != nil {
		defer lockedFlexvol.Unlock()
	}
...
func TestOntapSanEconomyEnablePublishEnforcement_FailsCheckingIfLUNExists(t *testing.T) {
	...
	mockAPI.EXPECT().LunList(ctx, gomock.Any()).Return(nil, errors.New("ontap api error"))
...
func TestCreateLunInternalID(t *testing.T) {
	_, driver := newMockOntapSanEcoDriver(t)

	svm := "fakeSVM"
	flexvol := "fakeFlexVol"
	lun := "fakeLun"
	testString := fmt.Sprintf("/svm/%s/flexvol/%s/lun/%s", svm, flexvol, lun)

	str := driver.CreateLUNInternalID(svm, flexvol, lun)

	assert.Equal(t, testString, str)
}

func TestParseLunInternalID(t *testing.T) {
	_, driver := newMockOntapSanEcoDriver(t)
	testString := "/svm/fakeSVM/flexvol/fakeFlexvol/lun/fakeLun"

	svm, flexvol, lun, err := driver.ParseLunInternalID(testString)
	...
	assert.Equal(t, lun, "fakeLun")
}

func TestParseLunInternalIdWithMissingSVM(t *testing.T) {
	_, driver := newMockOntapSanEcoDriver(t)
	testString := "/flexvol/fakeFlexvol/lun/fakeLun"

	_, _, _, err := driver.ParseLunInternalID(testString)
	...
	tests := []testCase{
		{
			...
			setupAPI: func(mockAPI *mockapi.MockOntapAPI) {
				...
				mockAPI.EXPECT().LunGetByName(ctx, gomock.Any()).Return(
					&api.Lun{Name: "/vol/volA/lunA_clone", Size: "1073741824", State: "online"}, nil)
