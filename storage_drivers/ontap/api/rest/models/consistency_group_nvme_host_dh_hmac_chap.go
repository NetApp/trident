...
func (m *ConsistencyGroupNvmeHostDhHmacChap) validateControllerSecretKey(formats strfmt.Registry) error {
	if swag.IsZero(m.ControllerSecretKey) { // not required
		return nil
	}

	if err := validate.FormatOf("controller_secret_key", "body", "password", m.ControllerSecretKey.String(), formats); err != nil {
		return err
	}
...
func (m *ConsistencyGroupNvmeHostDhHmacChap) validateHostSecretKey(formats strfmt.Registry) error {
	if swag.IsZero(m.HostSecretKey) { // not required
		return nil
	}

	if err := validate.FormatOf("host_secret_key", "body", "password", m.HostSecretKey.String(), formats); err != nil {
		return err
	}
