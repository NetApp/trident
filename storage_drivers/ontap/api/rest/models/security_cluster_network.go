...
type SecurityClusterNetwork struct {
	...
	// The certificate and TLS status of the cluster network security configuration.
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["tls","tls_ipsec"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// tls_ipsec
	// END DEBUGGING
	// SecurityClusterNetworkModeTLSIpsec captures enum value "tls_ipsec"
	SecurityClusterNetworkModeTLSIpsec string = "tls_ipsec"
)
