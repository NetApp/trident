...
func (o *NvmeSubsystemHostCreateReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (any, error) {
...
| 72090205 | An invalid combination for the TLS key type and configured PSK values was provided when adding an NVMe subsystem host. When key type is "none" or "generated", no configured PSK is allowed. When key type is "configured", a configured PSK is required. |
| 72090206 | A DH-HMAC-CHAP secret key is required while adding a host to a subsystem with a generated TLS key. If a DH-HMAC-CHAP group size is supplied, it must have a value other than 'none'. |
