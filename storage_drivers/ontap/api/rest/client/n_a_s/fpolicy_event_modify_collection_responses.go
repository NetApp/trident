...
func (o *FpolicyEventModifyCollectionReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (any, error) {
...
| 9765079    | The S3 protocol does not support the native engine |
| 9765080    | The S3 protocol does not support policies associated with the synchronous engine |
| 9765081    | The S3 protocol does not support mandatory screening set to true |
| 9765082    | The S3 protocol does not support the Persistent Store feature |
| 9765083    | The S3 protocol does not support Passthrough-read feature |
| 9765084    | The S3 protocol does not support the Resiliency feature |
| 9765086    | The S3 protocol does not support SSL connection to external server |
| 9765087    | The S3 protocol does not support notifications in XML format |
| 9765088    | The S3 protocol does not support with allow privileged access set to true |
*/
...
type FpolicyEventModifyCollectionBody struct {
	...
	//     * s3    - for the S3 protocol.
	//
	// Enum: ["cifs","nfsv3","nfsv4","s3"]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["cifs","nfsv3","nfsv4","s3"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// s3
	// END DEBUGGING
	// FpolicyEventModifyCollectionBodyProtocolS3 captures enum value "s3"
	FpolicyEventModifyCollectionBodyProtocolS3 string = "s3"
)
...
type FpolicyEventInlineFileOperations struct {
	...
	// Object create operations
	ObjectCreate *bool `json:"object_create,omitempty"`

	// Object delete operations
	ObjectDelete *bool `json:"object_delete,omitempty"`

	// Object tag operations
	ObjectTag *bool `json:"object_tag,omitempty"`
...
type FpolicyEventInlineFilters struct {
	...
	// Exclude the client request for objects with multiple delete operations.
	ExcludeMultiDelete *bool `json:"exclude_multi_delete,omitempty"`
	...
	// Filter the client requests for objects created with complete multipart upload operations.
	ObjectCreateWithCompleteMpu *bool `json:"object_create_with_complete_mpu,omitempty"`

	// Filter the client requests for objects created with copy operations.
	ObjectCreateWithCopy *bool `json:"object_create_with_copy,omitempty"`

	// Filter the client requests for objects created with PUT operations.
	ObjectCreateWithPut *bool `json:"object_create_with_put,omitempty"`

	// Filter the client requests for object tags with delete operations.
	ObjectTagDelete *bool `json:"object_tag_delete,omitempty"`

	// Filter the client requests for object tags with PUT operations.
	ObjectTagPut *bool `json:"object_tag_put,omitempty"`
